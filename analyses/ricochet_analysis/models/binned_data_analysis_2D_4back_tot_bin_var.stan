...
data{
  ...
  vector[nBins_0] x_back_3;
  vector[nBins_1] y_back_3;
  vector[nBins_0] x_back_4;
  vector[nBins_1] y_back_4;
...
parameters{
  ...
  real<lower=background_lb,upper=background_ub> background_rate_3;
  real<lower=background_lb,upper=background_ub> background_rate_4;
}
...
transformed parameters {
  ...
  background_rate = background_rate_1+background_rate_2+background_rate_3+background_rate_4;
  ...
  for(i in 1:nBins_0){
    ...
    for(j in 1:nBins_1) {
      ...
      n_counts_recon[i][j] = signal_rate_bins[i]*x_sig_1[i]*y_sig_1[j] +
	      		       background_rate_1*x_back_1[i]*y_back_1[j]+
                               background_rate_2*x_back_2[i]*y_back_2[j]+
			       background_rate_3*x_back_3[i]*y_back_3[j]+
			       background_rate_4*x_back_4[i]*y_back_4[j];
