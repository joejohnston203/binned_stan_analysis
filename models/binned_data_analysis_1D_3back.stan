...
* This model assumes there is one signal and two backgrounds,
...
data{
  ...
  // signal_rate*sig_1[i]
  //   + sum_l(back_rate*back_l[i])
  vector[nBins_0] sig_1;
  vector[nBins_0] back_1;
  vector[nBins_0] back_2;
  vector[nBins_0] back_3;
...
parameters{
  ...
  //vector[nBins_0] log_n_counts_recon_envelope; // gaussian envelope in each bin for n_counts_recon
  real<lower=background_lb,upper=background_ub> background_rate_1;   // Background event rate
  real<lower=background_lb,upper=background_ub> background_rate_2;   // Background event rate
  real<lower=background_lb,upper=background_ub> background_rate_3;   // Background event rate
}
...
transformed parameters {
  ...
  //vector[nBins_0] n_counts_recon_envelope;
  ...
  background_rate = background_rate_1+background_rate_2+background_rate_3;

  for(i in 1:nBins_0){
    signal_rate_bins_gauss[i] = exp(log_signal_rate_bins_gauss[i]);
    //n_counts_recon_envelope[i] = exp(log_n_counts_recon_envelope[i]);
    ...
    n_counts_recon[i] = signal_rate_bins[i]*sig_1[i]+
     			     background_rate_1*back_1[i]+
                             background_rate_2*back_2[i]+
                             background_rate_3*back_3[i];
    if (n_counts_recon[i]<0.){
      print(n_counts_recon[i]," Bin ", i);
    }
    //if(total_bin_variation!=0.0){
    //  n_counts_recon[i] = n_counts_recon[i]*n_counts_recon_envelope[i];
    //}
  }
...
model {
  ...
  //if(total_bin_variation!=0.0){
  //  increment_log_prob(sum(log_n_counts_recon_envelope)); // Jacobian
  //  n_counts_recon_envelope ~ normal(1.0,total_bin_variation);
  //}
