...
yaml_script_copy=./scripts/temp_60pctOn_4back_ricochet_rate_spec_convolved_nobinfluc2_analyzer.yaml

# Replace all instances of init_descriptor with new_descriptor in
# the .yaml script
init_descriptor='ric_rate_spec_example'
new_descriptor='ric_rate_spec_convolved_nobinfluc2_60pctOn_4back'
...
#              correlate_bins,correlate_days,fluc2tuation_frac]
...
sed -i 's/\(.*bins: \).*\( # Er\)/\1'$energy_bins'\2/' $yaml_script_copy
sed -i 's/\(.*\)recoilEnergies\(.*\)/\1xvals\2/' $yaml_script_copy
sed -i 's/\(.*\)differentialRates\(.*\)/\1yvals\2/' $yaml_script_copy

# Use convolved_nobinfluc2 exponential for energy dependence
