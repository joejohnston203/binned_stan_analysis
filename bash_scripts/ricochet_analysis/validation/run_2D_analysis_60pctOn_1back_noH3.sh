...
yaml_script=./scripts/ricochet_rate_spec_analyzer_noH3.yaml
yaml_script_copy=./scripts/temp_60pctOn_1back_noH3_ricochet_rate_spec_analyzer.yaml

# Replace all instances of init_descriptor with new_descriptor in
# the .yaml script
init_descriptor='ric_rate_spec_example'
new_descriptor='ric_rate_spec_60pctOn_noH3'
...
# Er Upper Bound (ROI was wrong in the initial runs)
Er_ub=3000
...
sed -i 's/\(.*upper_bound: \).*\( # Er\)/\1'$Er_ub'\2/' $yaml_script_copy
