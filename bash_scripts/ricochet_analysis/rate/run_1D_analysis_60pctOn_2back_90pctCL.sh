...
yaml_script_copy=./scripts/temp_60pctOn_2back_90pctCL_ricochet_rate_analyzer.yaml

# Replace all instances of init_descriptor with new_descriptor in
# the .yaml script
init_descriptor='ric_rate_example'
new_descriptor='ric_rate_60pctOn_2back_90pctCL'
...
# Quantiles to print on signal histograms
sig_quantile_fracs='[0.9]'
...
sed -i 's/\(.*print_quantile_fracs: \).*\( # Sig\)/\1'$sig_quantile_fracs'\2/' $yaml_script_copy
